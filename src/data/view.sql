SELECT 
        ...
        `audit_plan`.`rect_status` AS `rect_status`,
        `audit_plan`.`rectification_percentage` AS `rectification_percentage`,
        `audit_plan`.`audit_status` AS `audit_status`,
        `audit_status`.`status_name` AS `current_state`
    FROM
        (((((`audit_plan`
        ...
        LEFT JOIN `audit_status` ON ((`audit_plan`.`audit_status`=`audit_status`.`id`)))